...
const NFTContractComponent = () => {
  const [tokenId, setTokenId] = useState("");
  const [recipient, setRecipient] = useState("");
  const [tokenURI, setTokenURI] = useState("");
  const [newTokenURI, setNewTokenURI] = useState("");

  const contractAddress = "0xE76aDd852f7f8e25177A61B5daF5b4B47298F4ca"; // Replace with your contract address
  const abi = 
    [
  ...
  const handleMintNFT = async () => {
    try {
      const tx = await contract.mintNFT(recipient, tokenId, tokenURI);
      await tx.wait();
  ...
  const handleSetTokenURI = async () => {
    try {
      const tx = await contract.setTokenURI(tokenId, newTokenURI);
      await tx.wait();
...
export default NFTContractComponent;
